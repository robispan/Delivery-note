...
function fillToolFields(event, loadedData, suggestion) {
  ...
  let toolKey;
  if (identifier === 'name') {
    toolKey = Object.keys(loadedData.tools).filter(
      key => loadedData.tools[key].name === suggestion
    )[0];
  } else if (identifier === 'cod2') {
    toolKey = Object.keys(loadedData.tools).filter(
      key => loadedData.tools[key].code2 === suggestion
    )[0];
  } else toolKey = suggestion;
  ...
  $(document)
    .find(
      `.table-row:nth-child(${parseInt(rowNr) + 1}) .item:nth-child(5) input`
    )
    .focus();
}
...
function getNewPosition(keyCode, pos) {
  ...
  if (keyCode == '38') {
    ...
    if (pos.row === 2) newRow = rowCount;
    // Else jump to previous row
    else newRow = pos.row - 1;
  }

  // Down arrow
  else if (keyCode == '40') {
    // Jump to first row if on last row
    if (pos.row === rowCount) newRow = 2;
...
function navigateTable(event) {
  ...
  $(document)
    .find(`${newRowDiv} ${newColDiv} input`)
    .focus();
}
...
function togglePlusBtn(loadedData) {
  ...
  if (nameVal !== '' && code1val !== '' && code2val !== '') {
    ...
    // ... or if tool is listed and code2 or name inputs are edited
      ...
      loadedData.tools[code1val].name !== nameVal ||
